...
the output-contract boundary (Incident clamping), and the Adjudicator's
...
from vision.types import Detection, Incident, VerifiedObjects


def _detections(items=None):
    return VerifiedObjects(frame_index=0, items=items or [], dropped=0)
...
def test_adjudicator_uses_injected_provider():
    class StubProvider:
        def adjudicate(self, detections, context):
            return Incident(frame_index=0, risk=0.7, label="stub", summary="ok")
