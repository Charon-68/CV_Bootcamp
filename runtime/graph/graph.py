...
A Graph is a DAG of node instances connected by typed edges. Before
...
from runtime.nodes import Node, create
...
@dataclass
class Node:
    """A graph node: a unique id bound to a node instance."""
...
class Graph:
    ...
    def add_node(self, node_id: str, node: Node) -> Graph:
        ...
        self.nodes[node_id] = Node(id=node_id, node=node)
        return self
    ...
    def _check_endpoints(self) -> None:
        for e in self.edges:
            ...
            if not _has_port(self.nodes[e.src_node].node.outputs, e.src_port):
                raise GraphError(f"{e.src_node}: no output port {e.src_port!r}")
            if not _has_port(self.nodes[e.dst_node].node.inputs, e.dst_port):
                raise GraphError(f"{e.dst_node}: no input port {e.dst_port!r}")

    def _check_types(self) -> None:
        for e in self.edges:
            out = _port(self.nodes[e.src_node].node.outputs, e.src_port)
            inp = _port(self.nodes[e.dst_node].node.inputs, e.dst_port)
...
def build(spec: dict) -> Graph:
    ...
    Spec shape:
        {
          "nodes": [{"id": "cap", "node": "capture", "config": {...}}, ...],
          "edges": [{"src": "cap.frame", "dst": "clean.frame"}, ...],
        }
    """
    graph = Graph()
    for n in spec.get("nodes", []):
        node = create(n["node"], **n.get("config", {}))
        graph.add_node(n["id"], node)
