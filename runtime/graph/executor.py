...
the node's declared input ports, then passed to `node.run`.
...
from runtime.nodes import NodeResult
from runtime.graph.graph import Graph

logger = logging.getLogger("nexusguard.graph.executor")
...
class Executor:
    ...
    def run(self, seed: dict[str, dict[str, Any]] | None = None) -> dict[str, NodeResult]:
        ...
        for node_id in self.order:
            ...
            node.node.validate_inputs(inputs)
            logger.debug("running node %s (%s)", node_id, node.node.name)
            results[node_id] = node.node.run(inputs)

        return results

    def _gather_inputs(
        self, node_id: str, results: dict[str, NodeResult]
    ) -> dict[str, Any]:
